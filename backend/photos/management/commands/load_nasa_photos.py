...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        async def load_photos(session, job):
        ...
        async def get_jobs(session):
            ...
            for rover in rovers:
                ...
                job, _ = await Job.objects.select_related("rover").aget_or_create(
                    rover_id=rover["id"], defaults={"max_sol": rover["max_sol"]}
                )
                jobs.append(job)
            return jobs
        ...
        async def populate_db():
            async with ClientSession(json_serialize=json.dumps) as session:
                ...
                await asyncio.gather(*[load_photos(session, job) for job in jobs])
